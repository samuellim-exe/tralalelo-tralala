...
timeFont = pygame.font.Font(None, 26)
...
def display_data(wifi_connected, current_date, current_time, temperature, humidity, heat_index, light_intensity, uv_intensity, pressure, elevation, latitude, longitude, rating, message, air_quality, wind_speed, wind_direction, sunrise, sunset, clouds):
    ...
    text_surface = timeFont.render(date_time_str, True, (255, 255, 255))
    screen.blit(text_surface, (26, 8))

    # Display Wi-Fi status
    if wifi_connected:
        screen.blit(WifiIcon, (215, 5))

    pygame.draw.line(screen, (255, 255, 255), (0, 34), (screen_width, 34), 1)

    # Display environmental data
    screen.blit(TempIcon, (5, 35 + y_offset_1))
    text_surface = font.render(f"{temperature} 'C", True, (255, 255, 255))
    screen.blit(text_surface, (30, 40 + y_offset_1))

    screen.blit(HumIcon, (115, 35 + y_offset_1))
    text_surface = font.render(f"{humidity} %", True, (255, 255, 255))
    screen.blit(text_surface, (140, 40 +y_offset_1))

    # Uncommented code starts here and y-values adjusted for better positioning

    screen.blit(PressureIcon, (5, 65 + y_offset_1))  # Moved y from 95 to 65
    text_surface = font.render(f"{int(pressure)} hPa", True, (255, 255, 255))
    screen.blit(text_surface, (30, 70 + y_offset_1))  # Moved y from 100 to 70

    screen.blit(AltitudeIcon, (115, 65 + y_offset_1))  # Moved y from 95 to 65
    text_surface = font.render(f"{elevation} m", True, (255, 255, 255))
    screen.blit(text_surface, (140, 70 + y_offset_1))  # Moved y from 100 to 70

    screen.blit(LocationIcon, (5, 95 + y_offset_1))  # Moved y from 125 to 95
    text_surface = font.render(f"{latitude}, {longitude}", True, (255, 255, 255))
    screen.blit(text_surface, (30, 100 + y_offset_1))  # Moved y from 130 to 100

    if wifi_connected:
        screen.blit(HeatIcon, (5, 125 + y_offset_1))  # Moved y from 150 to 115
        text_surface = font.render(f"{heat_index} °C", True, (255, 255, 255))
        screen.blit(text_surface, (30, 130 + y_offset_1))  # Moved y from 155 to 120

        screen.blit(AirIcon, (115, 125 + y_offset_1))  # Moved y from 150 to 115
        text_surface = font.render(f"{air_quality}", True, (255, 255, 255))
        screen.blit(text_surface, (140, 130 + y_offset_1))  # Moved y from 155 to 120

        screen.blit(CloudsIcon, (5, 155 + y_offset_1))  # Moved y from 175 to 135
        text_surface = font.render(f"{clouds} %", True, (255, 255, 255))
        screen.blit(text_surface, (30, 160 + y_offset_1))  # Moved y from 180 to 140

        screen.blit(WindIcon, (115, 155 + y_offset_1))  # Moved y from 175 to 135
        text_surface = font.render(f"{wind_speed}, {wind_direction}", True, (255, 255, 255))
        screen.blit(text_surface, (140, 160 + y_offset_1))  # Moved y from 180 to 140

        screen.blit(SunriseIcon, (5, 185 + y_offset_1))  # Moved y from 200 to 155
        text_surface = font.render(f"{sunrise}", True, (255, 255, 255))
        screen.blit(text_surface, (30, 190 + y_offset_1))  # Moved y from 205 to 160

        screen.blit(SunsetIcon, (115, 185 + y_offset_1))  # Moved y from 225 to 175
        text_surface = font.render(f"{sunset}", True, (255, 255, 255))
        screen.blit(text_surface, (140, 190 + y_offset_1))  # Moved y from 230 to 180

    pygame.draw.line(screen, (255, 255, 255), (0, 215 + y_offset_1), (screen_width, 215 + y_offset_1), 1)  # Moved line to match new positioning

    # Display rating
    text_surface = font.render(f"{rating}", True, (255, 255, 255))
    screen.blit(text_surface, (0, 220 + y_offset_1))  # Moved y from 255 to 205

    # Display the evaluation message, wrapped for the screen width
    wrapped_lines = [message[i:i + 36] for i in range(0, len(message), 36)]
    y_offset = 235 + y_offset_1  # Moved offset to align with the new position
